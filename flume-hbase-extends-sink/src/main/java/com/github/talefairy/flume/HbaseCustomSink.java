...
/**
 * 自定义Sink接收器，它从一个通道读取事件并将其写入到HBase
 * Created by supoman on 2017/7/13.
 */
public class HbaseCustomSink extends AbstractSink implements Configurable {

    private static final Logger logger = LoggerFactory.getLogger(HbaseCustomSink.class);
    ...
    private HbaseCustomEventSerializer serializer;
    ...
    public HbaseCustomSink() {
        this(HBaseConfiguration.create());
    }

    public HbaseCustomSink(Configuration conf) {
        this.config = conf;
    }

    @VisibleForTesting
    @InterfaceAudience.Private
    HbaseCustomSink(Configuration conf, DebugIncrementsCallback cb) {
        this(conf);
    ...
    public void configure(Context context) {
        ...
        //从配置文件中获取指定的Serializer Event类（就是自定义的类）
        ...
        // 检查event序列化，如果不存在则使用默认的SimpleHbaseEventSerializer
        ...
        // 获取配置文件中的子属性： serializer.*   主要是用来设置列名，rowKey前缀，rowKey生成类型等。
        ...
        // 通过反射获取自定义的Event类类
        try {
            Class<? extends HbaseCustomEventSerializer> clazz = (Class<? extends HbaseCustomEventSerializer>)Class.forName(eventSerializerType);
        ...
        enableWal = context.getBoolean(HBaseSinkConfigurationConstants.CONFIG_ENABLE_WAL, HBaseSinkConfigurationConstants.DEFAULT_ENABLE_WAL);
    ...
    @VisibleForTesting
    @InterfaceAudience.Private
    HbaseCustomEventSerializer getSerializer() {
        return serializer;
    }
